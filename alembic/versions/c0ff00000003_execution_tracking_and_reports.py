...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "options", postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment="执行选项"
        ),
        sa.Column(
            "trigger_source",
            sa.String(length=50),
            nullable=True,
            comment="触发来源（api / scheduler）",
        ),
    ...
    op.create_index(
        "ix_node_executions_session_id", "node_executions", ["session_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False, comment="记录唯一标识"),
        sa.Column(
            "session_id", postgresql.UUID(as_uuid=True), nullable=True, comment="关联 session"
        ),
        sa.Column(
            "service_name",
            sa.String(length=50),
            nullable=False,
            comment="服务名（bochai / tushare / ...）",
        ),
        sa.Column(
            "operation", sa.String(length=100), nullable=False, comment="操作（web-search / ...）"
        ),
        sa.Column(
            "request_params",
            postgresql.JSONB(astext_type=sa.Text()),
...
def downgrade() -> None:
    op.drop_index(
        "ix_external_api_call_logs_session_id_created_at", table_name="external_api_call_logs"
    )
