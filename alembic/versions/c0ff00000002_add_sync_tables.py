...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("job_type", sa.String(length=50), nullable=False, comment="任务类型（DAILY_HISTORY/FINANCE_HISTORY/DAILY_INCREMENTAL/FINANCE_INCREMENTAL）"),
        sa.Column("status", sa.String(length=20), nullable=False, comment="任务状态（PENDING/RUNNING/COMPLETED/FAILED/PAUSED）"),
        sa.Column("current_offset", sa.Integer(), nullable=False, server_default="0", comment="当前同步偏移量（用于分批处理）"),
        sa.Column("batch_size", sa.Integer(), nullable=False, server_default="50", comment="每批处理的股票数量"),
        sa.Column("total_processed", sa.Integer(), nullable=False, server_default="0", comment="已处理总条数"),
        ...
        sa.Column("config", postgresql.JSON(astext_type=sa.Text()), nullable=True, comment="任务特定配置（start_date、end_date 等）"),
    ...
    op.create_table(
        ...
        sa.Column("job_type", sa.String(length=50), nullable=False, comment="任务类型（DAILY_HISTORY/FINANCE_HISTORY/DAILY_INCREMENTAL/FINANCE_INCREMENTAL）"),
        sa.Column("third_code", sa.String(length=20), nullable=False, comment="失败的股票代码（Tushare ts_code 格式）"),
        sa.Column("error_message", sa.String(length=500), nullable=True, comment="错误信息"),
        sa.Column("retry_count", sa.Integer(), nullable=False, server_default="0", comment="当前重试次数"),
        ...
        sa.Column("resolved_at", sa.DateTime(), nullable=True, comment="解决时间（重试成功或人工标记）"),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(op.f("ix_sync_failure_records_job_type"), "sync_failure_records", ["job_type"], unique=False)
    op.create_index(op.f("ix_sync_failure_records_third_code"), "sync_failure_records", ["third_code"], unique=False)
    op.create_index(op.f("ix_sync_failure_records_resolved_at"), "sync_failure_records", ["resolved_at"], unique=False)
