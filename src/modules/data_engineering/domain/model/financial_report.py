...
class StockFinance(BaseEntity):
    ...
    undist_profit_ps: Optional[float] = Field(
        None, description="每股未分配利润"
    )
    extra_item: Optional[float] = Field(None, description="非经常性损益")
    profit_dedt: Optional[float] = Field(
        None, description="扣除非经常性损益后的净利润"
    )
    ...
    valuechange_income: Optional[float] = Field(
        None, description="价值变动净收益"
    )
    ...
    noncurrent_exint: Optional[float] = Field(
        None, description="无息非流动负债"
    )
    ...
    networking_capital: Optional[float] = Field(
        None, description="营运流动资本"
    )
    ...
    ocfps: Optional[float] = Field(
        None, description="每股经营活动产生的现金流量净额"
    )
    ...
    expense_of_sales: Optional[float] = Field(
        None, description="销售期间费用率"
    )
    profit_to_gr: Optional[float] = Field(
        None, description="净利润/营业总收入"
    )
    saleexp_to_gr: Optional[float] = Field(
        None, description="销售费用/营业总收入"
    )
    adminexp_of_gr: Optional[float] = Field(
        None, description="管理费用/营业总收入"
    )
    finaexp_of_gr: Optional[float] = Field(
        None, description="财务费用/营业总收入"
    )
    impai_ttm: Optional[float] = Field(
        None, description="资产减值损失/营业总收入"
    )
    gc_of_gr: Optional[float] = Field(
        None, description="营业总成本/营业总收入"
    )
    op_of_gr: Optional[float] = Field(None, description="营业利润/营业总收入")
    ebit_of_gr: Optional[float] = Field(
        None, description="息税前利润/营业总收入"
    )
    roe: Optional[float] = Field(None, description="净资产收益率")
    roe_waa: Optional[float] = Field(None, description="加权平均净资产收益率")
    roe_dt: Optional[float] = Field(
        None, description="净资产收益率(扣除非经常损益)"
    )
    ...
    opincome_of_ebt: Optional[float] = Field(
        None, description="经营活动净收益/利润总额"
    )
    investincome_of_ebt: Optional[float] = Field(
        None, description="价值变动净收益/利润总额"
    )
    n_op_profit_of_ebt: Optional[float] = Field(
        None, description="营业外收支净额/利润总额"
    )
    tax_to_ebt: Optional[float] = Field(None, description="所得税/利润总额")
    dtprofit_to_profit: Optional[float] = Field(
        None, description="扣除非经常损益后的净利润/净利润"
    )
    salescash_to_or: Optional[float] = Field(
        None, description="销售商品提供劳务收到的现金/营业收入"
    )
    ocf_to_or: Optional[float] = Field(
        None, description="经营活动产生的现金流量净额/营业收入"
    )
    ocf_to_opincome: Optional[float] = Field(
        None, description="经营活动产生的现金流量净额/经营活动净收益"
    )
    capitalized_to_da: Optional[float] = Field(
        None, description="资本支出/折旧和摊销"
    )
    debt_to_assets: Optional[float] = Field(None, description="资产负债率")
    assets_to_eqt: Optional[float] = Field(None, description="权益乘数")
    dp_assets_to_eqt: Optional[float] = Field(
        None, description="权益乘数(杜邦分析)"
    )
    ca_to_assets: Optional[float] = Field(None, description="流动资产/总资产")
    nca_to_assets: Optional[float] = Field(
        None, description="非流动资产/总资产"
    )
    tbassets_to_totalassets: Optional[float] = Field(
        None, description="有形资产/总资产"
    )
    int_to_talcap: Optional[float] = Field(
        None, description="带息债务/全部投入资本"
    )
    eqt_to_talcapital: Optional[float] = Field(
        None, description="归属于母公司的股东权益/全部投入资本"
    )
    currentdebt_to_debt: Optional[float] = Field(
        None, description="流动负债/负债合计"
    )
    longdeb_to_debt: Optional[float] = Field(
        None, description="非流动负债/负债合计"
    )
