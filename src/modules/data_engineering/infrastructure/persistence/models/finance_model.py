...
class StockFinanceModel(Base):
    ...
    ann_date = Column(
        Date, primary_key=True, nullable=False, index=True, comment="公告日期"
    )

    end_date = Column(
        Date, primary_key=True, nullable=False, index=True, comment="报告期"
    )
    ...
    profit_dedt = Column(
        Float, nullable=True, comment="扣除非经常性损益后的净利润"
    )
    ...
    ocfps = Column(
        Float, nullable=True, comment="每股经营活动产生的现金流量净额"
    )
    ...
    adminexp_of_gr = Column(
        Float, nullable=True, comment="管理费用/营业总收入"
    )
    ...
    roe_dt = Column(
        Float, nullable=True, comment="净资产收益率(扣除非经常损益)"
    )
    ...
    opincome_of_ebt = Column(
        Float, nullable=True, comment="经营活动净收益/利润总额"
    )
    investincome_of_ebt = Column(
        Float, nullable=True, comment="价值变动净收益/利润总额"
    )
    n_op_profit_of_ebt = Column(
        Float, nullable=True, comment="营业外收支净额/利润总额"
    )
    tax_to_ebt = Column(Float, nullable=True, comment="所得税/利润总额")
    dtprofit_to_profit = Column(
        Float, nullable=True, comment="扣除非经常损益后的净利润/净利润"
    )
    salescash_to_or = Column(
        Float, nullable=True, comment="销售商品提供劳务收到的现金/营业收入"
    )
    ocf_to_or = Column(
        Float, nullable=True, comment="经营活动产生的现金流量净额/营业收入"
    )
    ...
    capitalized_to_da = Column(
        Float, nullable=True, comment="资本支出/折旧和摊销"
    )
    ...
    dp_assets_to_eqt = Column(
        Float, nullable=True, comment="权益乘数(杜邦分析)"
    )
    ...
    tbassets_to_totalassets = Column(
        Float, nullable=True, comment="有形资产/总资产"
    )
    int_to_talcap = Column(
        Float, nullable=True, comment="带息债务/全部投入资本"
    )
    eqt_to_talcapital = Column(
        Float, nullable=True, comment="归属于母公司的股东权益/全部投入资本"
    )
    currentdebt_to_debt = Column(
        Float, nullable=True, comment="流动负债/负债合计"
    )
    longdeb_to_debt = Column(
        Float, nullable=True, comment="非流动负债/负债合计"
    )
    ocf_to_shortdebt = Column(
        Float, nullable=True, comment="经营活动产生的现金流量净额/流动负债"
    )
