...
class SyncFailureRecordModel(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, comment="失败记录 ID")
    ...
    third_code = Column(String(20), nullable=False, index=True, comment="失败的股票代码（Tushare ts_code 格式）")
    ...
    resolved_at = Column(DateTime, nullable=True, index=True, comment="解决时间（重试成功或人工标记）")
