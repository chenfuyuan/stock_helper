...
from .persistence.repositories.scheduler_execution_log_repo import SchedulerExecutionLogRepository
...
class ExecutionTracker:
    ...
    async def __aexit__(
        ...
        if exc_type is None:
            ...
            status = "SUCCESS"
            error_message = None
            logger.info(f"调度任务 {self._job_id} 执行成功，耗时 {duration_ms}ms")
        else:
            # 异常退出：FAILED
            status = "FAILED"
            error_message = f"{exc_type.__name__}: {str(exc_val)}"
            logger.error(f"调度任务 {self._job_id} 执行失败: {error_message}，耗时 {duration_ms}ms")
