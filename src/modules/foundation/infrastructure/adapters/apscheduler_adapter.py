...
from src.modules.foundation.domain.types import JobFunction
...
class APSchedulerAdapter(SchedulerPort):
        ...
        **kwargs,
        ...
        try:
            ...
            raise SchedulerExecutionException(
                job_id=job_id, error_message=f"调度任务失败: {str(e)}", original_error=e
            )
    ...
    async def start_scheduler(self) -> None:
        ...
        try:
            ...
            raise SchedulerException(
                message=f"启动调度器失败: {str(e)}", code="SCHEDULER_START_FAILED"
            )
    ...
    async def shutdown_scheduler(self) -> None:
        ...
        try:
            ...
            raise SchedulerException(
                message=f"关闭调度器失败: {str(e)}", code="SCHEDULER_SHUTDOWN_FAILED"
            )
    ...
    async def trigger_job(self, job_id: str, **kwargs) -> None:
        ...
        try:
            ...
            raise SchedulerExecutionException(
                job_id=job_id, error_message=f"触发任务失败: {str(e)}", original_error=e
            )
