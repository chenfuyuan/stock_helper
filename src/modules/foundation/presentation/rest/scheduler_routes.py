...
async def schedule_job(
    ...
    try:
        ...
        if request.job_id not in job_registry:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"任务 {request.job_id} 不在注册表中"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"调度任务失败: {str(e)}"
        )
...
async def start_job(
    ...
    try:
        ...
        if job_id not in job_registry:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"任务 {job_id} 不在注册表中"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"启动任务失败: {str(e)}"
        )
...
async def stop_job(
    job_id: str, scheduler_service: SchedulerApplicationService = Depends(get_scheduler_service)
):
    """停止指定任务并标记为禁用"""
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"停止任务失败: {str(e)}"
        )
...
async def trigger_job(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"触发任务失败: {str(e)}"
        )
...
async def get_executions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"查询执行历史失败: {str(e)}"
        )
