...
from src.modules.foundation.domain.types import JobId, JobRegistry
...
class SchedulerApplicationService:
    ...
    async def schedule_job(self, job_config: JobConfigDTO, job_registry: JobRegistry) -> None:
        """调度定时任务
        ...
        try:
            ...
            logger.info(f"任务调度成功: {job_config.job_id} - {job_config.cron_expression}")
            ...
            raise SchedulerExecutionException(
                job_id=job_config.job_id, error_message=f"调度任务失败: {str(e)}", original_error=e
            )
    ...
    async def get_job_status(self, job_id: JobId) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            raise SchedulerExecutionException(
                job_id=job_id, error_message=f"获取任务状态失败: {str(e)}", original_error=e
            )
    ...
    async def get_all_jobs_status(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise SchedulerExecutionException(
                job_id=None, error_message=f"获取所有任务状态失败: {str(e)}", original_error=e
            )
    ...
    async def remove_job(self, job_id: JobId) -> None:
        ...
        try:
            ...
            raise SchedulerExecutionException(
                job_id=job_id, error_message=f"移除任务失败: {str(e)}", original_error=e
            )
        ...
        try:
            ...
            for config in configs:
                ...
                if job_id not in job_registry:
                    logger.warning(f"跳过调度配置 {job_id}：在任务注册表中未找到对应的任务函数")
                    skipped_count += 1
                    continue
            ...
            logger.info(f"调度配置加载完成：成功 {loaded_count} 个，跳过 {skipped_count} 个")
    ...
    def _validate_job_config(self, job_config: JobConfigDTO) -> None:
        ...
        if not job_config.job_id or not job_config.job_id.strip():
            raise SchedulerConfigurationException(
                config_key="job_id", config_value=job_config.job_id, reason="任务ID不能为空"
            )

        if not job_config.job_name or not job_config.job_name.strip():
            raise SchedulerConfigurationException(
                config_key="job_name", config_value=job_config.job_name, reason="任务名称不能为空"
            )
    ...
    async def update_job_config(
        self, job_id: JobId, job_config: JobConfigDTO, job_registry: JobRegistry
    ) -> None:
        """更新任务配置
    ...
    async def schedule_and_persist_job(
        self, job_config: JobConfigDTO, job_registry: JobRegistry
    ) -> None:
        """调度并持久化任务（原子编排）
    ...
    async def stop_and_disable_job(self, job_id: JobId) -> None:
        ...
        try:
            ...
            raise SchedulerExecutionException(
                job_id=job_id, error_message=f"停止并禁用任务失败: {str(e)}", original_error=e
            )
    ...
    async def trigger_job(self, job_id: JobId, **kwargs) -> None:
        ...
        try:
            ...
            raise SchedulerExecutionException(
                job_id=job_id, error_message=f"手动触发任务失败: {str(e)}", original_error=e
            )

    async def query_execution_logs(
        self, job_id: Optional[JobId] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """查询任务执行历史
        ...
        try:
            if job_id:
                ...
                logs = await self._scheduler_execution_log_repo_port.get_recent(limit=limit)
            ...
            raise SchedulerExecutionException(
                job_id=job_id, error_message=f"查询执行日志失败: {str(e)}", original_error=e
            )
