...
class Neo4jGraphRepository(IGraphRepository):
    ...
    async def ensure_constraints(self) -> None:
        ...
        """
        constraints = [
            "CREATE CONSTRAINT stock_third_code_unique IF NOT EXISTS "
            "FOR (s:STOCK) REQUIRE s.third_code IS UNIQUE",
            ...
            "CREATE CONSTRAINT concept_code_unique IF NOT EXISTS "
    ...
    async def find_neighbors(
        ...
        if dimension == "industry":
            cypher = """
            MATCH (s:STOCK {third_code: $third_code})
            -[:BELONGS_TO_INDUSTRY]->(i:INDUSTRY)
            <-[:BELONGS_TO_INDUSTRY]-(neighbor:STOCK)
            ...
            cypher = """
            MATCH (s:STOCK {third_code: $third_code})
            -[:BELONGS_TO_CONCEPT]->(c:CONCEPT {name: $dimension_name})
            <-[:BELONGS_TO_CONCEPT]-(neighbor:STOCK)
            WHERE neighbor.third_code <> $third_code
            RETURN neighbor.third_code AS third_code,
                   neighbor.name AS name,
                   NULL AS industry,
            ...
            with self._driver.session() as session:
                ...
                for i in range(0, total, batch_size):
                    ...
                    try:
                        ...
                        success += len(batch)
                        logger.info(
                            f"成功同步概念批次 {i // batch_size + 1}，共 {len(batch)} 条记录"
                        )
                    except Exception as e:
                        logger.warning(
                        ...
                        for concept_payload in batch_data:
                            ...
                            try:
                                ...
                                failed += 1
                                error_msg = (
                                    f"concept_code={concept_code} 同步失败: {str(single_error)}"
                                )
                                error_details.append(error_msg)
                                logger.error(error_msg)
    ...
    async def clear_all_graph_data(self) -> dict:
        ...
        try:
            with self._driver.session() as session:
                ...
                logger.info(
                    f"图谱清空完成，已删除 {node_count} 个节点和 {relationship_count} 条关系"
                )
