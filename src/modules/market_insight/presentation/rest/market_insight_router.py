...
@router.get(
    ...
    response_model=BaseResponse[List[ConceptHeatDTO]],
    summary="查询概念热度",
...
) -> BaseResponse[List[ConceptHeatDTO]]:
...
@router.get(
    ...
    response_model=BaseResponse[List[LimitUpStockDTO]],
    summary="查询涨停股",
...
async def get_limit_up(
    ...
    container: MarketInsightContainer = Depends(get_container),
) -> BaseResponse[List[LimitUpStockDTO]]:
...
@router.post(
    ...
    response_model=BaseResponse[DailyReportResult],
    summary="生成每日复盘报告",
...
async def post_daily_report(
    trade_date: date = Query(..., description="交易日期"),
    container: MarketInsightContainer = Depends(get_container),
) -> BaseResponse[DailyReportResult]:
...
@router.get(
    ...
    response_model=BaseResponse[SentimentMetricsDTO],
    summary="查询市场情绪指标",
...
async def get_sentiment_metrics(
    trade_date: date = Query(..., description="交易日期"),
    container: MarketInsightContainer = Depends(get_container),
) -> BaseResponse[SentimentMetricsDTO]:
...
@router.get(
    ...
    response_model=BaseResponse[CapitalFlowAnalysisDTO],
    summary="查询资金流向分析",
...
async def get_capital_flow_analysis(
    ...
    container: MarketInsightContainer = Depends(get_container),
) -> BaseResponse[CapitalFlowAnalysisDTO]:
