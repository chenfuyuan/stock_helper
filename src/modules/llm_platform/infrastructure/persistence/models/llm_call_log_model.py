...
class LLMCallLogModel(Base):
    ...
    session_id = Column(UUID(as_uuid=True), nullable=True, index=True, comment="关联 session（无上下文时为 null）")
    caller_module = Column(String(50), nullable=False, comment="调用方模块名")
    caller_agent = Column(String(50), nullable=True, comment="调用方 Agent 标识")
