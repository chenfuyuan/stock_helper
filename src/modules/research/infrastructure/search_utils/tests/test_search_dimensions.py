...
def count_core_keywords(query_template: str) -> int:
    ...
    # 移除占位符
    cleaned = (
        query_template.replace("{stock_name}", "")
        .replace("{industry}", "")
...
class TestMacroSearchDimensions:
    def test_macro_dimensions_compliance(self):
        ...
        for config in MACRO_SEARCH_DIMENSIONS:
            ...
            assert (
                4 <= config.count <= 10
            ), f"宏观维度 {config.topic} 的 count {config.count} 不在 4-10 范围内"
            ...
            assert (
                config.freshness in valid_freshness
            ), f"宏观维度 {config.topic} 的 freshness {config.freshness} 不是合法值"

            # 检查核心关键词数量
            core_keywords = count_core_keywords(config.query_template)
            assert (
                core_keywords <= 3
            ), f"宏观维度 {config.topic} 的查询模板核心关键词数量 {core_keywords} 超过 3 个"
...
class TestCatalystSearchDimensions:
    def test_catalyst_dimensions_compliance(self):
        ...
        for config in CATALYST_SEARCH_DIMENSIONS:
            ...
            assert (
                4 <= config.count <= 10
            ), f"催化剂维度 {config.topic} 的 count {config.count} 不在 4-10 范围内"
            ...
            assert (
                config.freshness in valid_freshness
            ), f"催化剂维度 {config.topic} 的 freshness {config.freshness} 不是合法值"

            # 检查核心关键词数量
            core_keywords = count_core_keywords(config.query_template)
            assert (
                core_keywords <= 3
            ), f"催化剂维度 {config.topic} 的查询模板核心关键词数量 {core_keywords} 超过 3 个"
