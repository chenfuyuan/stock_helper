...
class CatalystDetectiveService:
    ...
    async def run(self, symbol: str) -> dict[str, Any]:
        ...
        # 2. 从 Web 搜索获取催化剂上下文
        search_results: List[CatalystSearchResult] = await self.data_port.search_catalyst_context(
        ...
        # 4. 构建上下文
        context_dto: CatalystContextDTO = self.context_builder.build(overview, search_results)
