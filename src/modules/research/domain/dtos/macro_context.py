...
class MacroContextDTO(BaseModel):
    ...
    current_date: str = Field(
        ..., description="当前日期（格式如'2026-02-13'）"
    )
    monetary_context: str = Field(
        ..., description="货币与流动性维度的搜索情报文本"
    )
    policy_context: str = Field(
        ..., description="产业政策与监管维度的搜索情报文本"
    )
    economic_context: str = Field(
        ..., description="宏观经济周期维度的搜索情报文本"
    )
    industry_context: str = Field(
        ..., description="行业景气与资金流向维度的搜索情报文本"
    )
    all_source_urls: str = Field(
        ..., description="所有搜索结果的来源 URL 列表（格式化为字符串）"
    )
