...
class TechnicalIndicatorsSnapshot(BaseModel):
    ...
    rsi_value: Optional[float] = Field(
        default=None, description="RSI(14)，数据不足时为 None"
    )
    ...
    macd_histogram: Optional[float] = Field(
        default=None, description="MACD 柱状图 DIF-DEA"
    )
    ...
    volume_ratio: Optional[float] = Field(
        default=None, description="量比（当日量/5日均量）"
    )
    obv_trend: str = Field(
        default="", description="OBV 5日趋势：Rising / Falling / Flat"
    )
    vwap_value: Optional[float] = Field(
        default=None, description="成交量加权均价（周期内 VWAP）"
    )
    price_vs_vwap_status: str = Field(
        default="", description="当前价相对 VWAP：上方/下方/持平"
    )
    bb_upper: Optional[float] = Field(
        default=None, description="布林带上轨(20,2)"
    )
    bb_lower: Optional[float] = Field(
        default=None, description="布林带下轨(20,2)"
    )
    bb_middle: Optional[float] = Field(
        default=None, description="布林带中轨 MA20"
    )
    bb_bandwidth: Optional[float] = Field(
        default=None, description="布林带宽(%)"
    )
    atr_value: Optional[float] = Field(
        default=None, description="ATR(14) 平均真实波幅"
    )
    high_20d: float = Field(default=0.0, description="近 20 日最高价")
    low_20d: float = Field(default=0.0, description="近 20 日最低价")
    calculated_support_levels: List[float] = Field(
        default_factory=list, description="支撑位列表"
    )
    calculated_resistance_levels: List[float] = Field(
        default_factory=list, description="阻力位列表"
    )
    detected_patterns: List[str] = Field(
        default_factory=list, description="识别到的 K 线形态"
    )
