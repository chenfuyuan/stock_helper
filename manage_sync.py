...
async def main():
    ...
    sync_finance_parser = subparsers.add_parser(
        "sync_finance", help="Sync incremental finance data"
    )
    sync_finance_parser.add_argument(
        "--date", type=str, help="Specific date to sync (YYYYMMDD), defaults to today"
    )

    # Sync Daily Bar Command
    sync_daily_parser = subparsers.add_parser("sync_daily", help="Sync incremental daily bar data")
    sync_daily_parser.add_argument(
        "--date", type=str, help="Specific date to sync (YYYYMMDD), defaults to today"
    )
