...
class TestScheduleAndPersistJob:
    ...
    async def test_schedule_and_persist_job_rollback_on_persistence_failure(
        ...
        # 验证异常消息
        assert "持久化失败" in str(exc_info.value.message) or "DB 写入失败" in str(
...
class TestQueryExecutionLogs:
    ...
    @pytest.mark.asyncio
    async def test_query_execution_logs_by_job_id(self, scheduler_service, mock_execution_log_repo):
        """按 job_id 查询执行日志"""
        job_id = "test_job_123"
        mock_logs = [
            {
    ...
    @pytest.mark.asyncio
    async def test_query_execution_logs_recent(self, scheduler_service, mock_execution_log_repo):
    ...
    @pytest.mark.asyncio
    async def test_query_execution_logs_empty(self, scheduler_service, mock_execution_log_repo):
        """查询结果为空"""
...
class TestRemoveJobNoHasattr:
    ...
    @pytest.mark.asyncio
    async def test_remove_job_calls_port_directly(self, scheduler_service, mock_scheduler_port):
    ...
    @pytest.mark.asyncio
    async def test_remove_job_propagates_exception(self, scheduler_service, mock_scheduler_port):
