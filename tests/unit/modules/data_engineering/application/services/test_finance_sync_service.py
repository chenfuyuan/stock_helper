...
async def test_run_incremental_sync_with_target_date(
    ...
    with patch(
        "src.modules.data_engineering.application.services.finance_sync_service.SyncUseCaseFactory"
    ) as mock_factory:
...
async def test_run_incremental_sync_without_target_date(
    ...
    with patch(
        ...
        # 模拟当前日期
        with patch(
            "src.modules.data_engineering.application.services.finance_sync_service.datetime"
