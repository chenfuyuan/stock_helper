# Spec: research-financial-auditor
...
财务审计员所需的结构化数据（静态快照 + 历史趋势）SHALL 通过 Research Domain 层定义的「构建财务快照」Port（如 `IFinancialSnapshotBuilder`）获得：该 Port 入参为多期财务数据 DTO 列表，出参为与 User Prompt 模板一致的 `FinancialSnapshotDTO`。Port 的具体实现 SHALL 位于 Research 的 Infrastructure 层。实现 SHALL 包含以下处理：
...
财务审计员的 System Prompt 与 User Prompt 模板 SHALL 内聚在该专家（agent）的上下文中，且 SHALL NOT 将 prompt 正文硬编码在代码中。Prompt 内容 SHALL 存放在代码库内约定的资源目录（`src/modules/research/agents/financial_auditor/prompts/` 下的 `system.md` 与 `user.md`），由该专家的 Infrastructure 在运行时加载；模板中的占位符（如 `{symbol}`、`{gross_margin}`、`{roic_series}`）SHALL 在代码中用本次调用的输入数据填充。
