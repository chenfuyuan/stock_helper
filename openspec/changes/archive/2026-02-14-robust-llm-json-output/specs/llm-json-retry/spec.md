...
带错误反馈的 LLM JSON 重试能力——统一处理器提供 `generate_and_parse[T]` 异步函数，封装 调用LLM → 解析 → 失败时带错误信息重试 的闭环。重试逻辑内聚于处理器，不修改 `ILLMProvider` 或 `LLMService` 接口。
...
- `llm_call: Callable[..., Awaitable[str]]` — LLM 回调函数（如 `functools.partial(llm_service.generate, alias="xxx")`）
...
重试时 `system_message` 和 `temperature` SHALL 与原始调用保持一致。
...
`generate_and_parse` 的每次 `llm_call` 调用（包括重试）SHALL 独立经过 `LLMService` 的调用审计链路。本函数不额外实现审计逻辑——审计由 `llm_call` 内部的 `LLMService.generate` 自然完成。
...
当 `llm_call` 本身抛出异常（如 `LLMConnectionError`、网络超时）时，`generate_and_parse` SHALL 直接向上透传该异常，不进行重试。重试仅针对解析失败（`LLMJsonParseError`），不针对 LLM 调用失败。
