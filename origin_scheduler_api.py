...
from src.modules.market_data.presentation.cron.sync_job import (
    sync_history_daily_data_job,
    sync_daily_by_date_job,
    sync_history_finance_job,
...
@router.get(
    "/status", response_model=BaseResponse[SchedulerStatusResponse], summary="获取调度器状态"
)
...
async def start_job(
    job_id: str, interval_minutes: int = Body(..., embed=True, description="执行间隔(分钟)")
):
    """
    ...
    scheduler.add_job(
        job_func, "interval", minutes=interval_minutes, id=job_id, replace_existing=True
    )
...
async def schedule_job(
    ...
    scheduler.add_job(job_func, "cron", hour=hour, minute=minute, id=job_id, replace_existing=True)
...
@router.post("/jobs/{job_id}/trigger", response_model=BaseResponse[str], summary="立即触发一次任务")
async def trigger_job(
    job_id: str, params: Optional[Dict[str, Any]] = Body(None, description="任务参数")
):
    """
    立即异步执行一次任务
    """
    ...
    return BaseResponse(
        success=True, code="JOB_TRIGGERED", message=f"已触发任务 '{job_id}'", data="triggered"
    )


@router.post("/jobs/{job_id}/stop", response_model=BaseResponse[str], summary="停止指定定时任务")
